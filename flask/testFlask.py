...
def run_script():
    ...
    if not file_path:
        return jsonify({"output": "No file provided"}), 400

    # Use the file in your logic here
    try:
        print("trying ", file_path)
        # with open(file_path, 'r') as f:
        #     content = f.read()
        #     result = {"output": f"File content: {content}"}
        text = ""
        ...
        result = json.dumps(studentJSON)

    except Exception as e:
        result = {"output": f"Error reading file: {str(e)}"}
